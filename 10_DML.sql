...
WHEN SALARY >= 8000000 THEN
    INTO TEMP1(ID, PW) VALUES ('동일이 급여 팔백이상' , '1234')
WHEN SALARY < 8000000 THEN
    INTO TEMP2(ID, PW) VALUES ('동일이 급여 팔백미만' , '1234')
(SELECT * FROM EMPLOYEE WHERE EMP_NAME = '선동일')
;

ROLLBACK;
-- DDL (CREATE TABLE) 이후 모두 취소
-- DDL 실행후에 COMMIT이 찍혀서
-- COMMIT 안찍은 DML은 반영 안되었음,

COMMIT;
-- 실제 DB에 반영.
-- COMMIT 을 찍어야 DB에 반영이되고 ROLLBACK 해도 DML 실행 결과가 남아있음.
-- 일종의 세이브 포인트 역할도 함.
-- DDL 실행시 자동으로 찍힘.
